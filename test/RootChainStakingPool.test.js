...
const RootChainStakingPool = artifacts.require('./RootChainStakingPool');
...
contract('RootChainStakingPool', async (accounts) => {
  let pool;
  ...
  beforeEach(async () => {
    pool = await RootChainStakingPool.new(
      miner,
      minerContactInfo,
      admin,
      adminContactInfo,
  ...
  it('should calculate dividends correctly', async () => {
    ...
    assert.equal(stakesWithDividends, '45999999999999999996');
    await pool.withdrawStakes(toWei(46.1)).should.be.rejectedWith(revertError);
    await pool.withdrawStakes('45999999999999999996');
    ...
    poolBalance = await web3.eth.getBalance(pool.address);
    assert.equal(poolBalance, '4000000000000000004');
    // Miner can withdraw as well.
    await pool.withdrawMinerReward({ from: miner });
    poolBalance = await web3.eth.getBalance(pool.address);
    assert.equal(poolBalance, 4);
    minerReward = await pool.minerReward();
    assert.equal(minerReward, 0);
  });
  ...
  it('should accept new stakers', async () => {
    ...
    assert.equal(stakerInfo[1], toWei(10));
    ...
    assert.equal(stakerInfo[1], toWei(2.5));
    ...
    assert.equal(stakerInfo[1], toWei(10.8));

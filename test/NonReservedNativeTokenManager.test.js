...
contract('NonReservedNativeTokenManager', async (accounts) => {
  ...
  it('should handle new token bid successfully', async () => {
    ...
    // ----------------------- ROUND 1 -----------------------
    // Bidder 2 places a bid with a small token id, should fail.
    await manager.bidNewToken(1000, toWei(20), 1, { from: accounts[2], value: toWei(20) })
      .should.be.rejectedWith(revertError);
    // Bidder 2 places a valid bid, should succeed.
